...
export default function FloatingNavMenu({ children }: PropsWithChildren) {
    ...
    useEffect(() => {
        if (!hiddenDivRef.current || !floatingDivRef.current) {
            return
        }

        hiddenDivRef.current.style.height = `${floatingDivRef.current.clientHeight}px`
    }, [])
    ...
    return (
        <>
            <div
                ref={floatingDivRef}
                className={`absolute bg-white rounded-lg shadow-lg transition-all duration-300 cursor-pointer ${
